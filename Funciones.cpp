...
void lecturaArchivo(Grafo *ptrGrafo){
    ...
    dato *auxDatosPartida, *auxDatosLlegada;
    ...
    while (archivo >> claveSalida){
        ...
        archivo >> datoString;
        limpiarGuiones(ptrDato);
        auxDatosLlegada->nombre = datoString;
        archivo >> datoString;
        limpiarGuiones(ptrDato);
        auxDatosLlegada->ciudad = datoString;
        archivo >> datoString;
        limpiarGuiones(ptrDato);
        auxDatosLlegada->pais = datoString;
        archivo >> costo;
        ptrAeropuertoLlegada= new Aeropuerto(claveLlegada,auxDatosLlegada);
        ptrVuelo = new Vuelo(costo,ptrAeropuertoLlegada);
        ptrAeropuertoSalida = new Aeropuerto(claveSalida,auxDatosPartida,ptrVuelo,ptrAeropuertoLlegada);
        ptrGrafo->asignarSiguienteAeropuerto(ptrAeropuertoSalida);
    }
	archivo.close();
